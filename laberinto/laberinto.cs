...
public class Laberinto
{
    ...
    private void GenerarLaberinto(int x, int y)
    {
       ...
       private void GenerarTrampasAleatorias(int cantidad)
    {
        Random rand = new Random();
        for (int i = 0; i < cantidad; i++)
        {
            int x = rand.Next(filas);
            int y = rand.Next(columnas);

            // Asegurarse de que la posición de la trampa no sea la de victoria ni una pared
            if (laberinto[x, y] == ' ' && (x != PosicionVictoria.x || y != PosicionVictoria.y))
            {
                AgregarTrampa(x, y);
            }
        }
    ...
    public void AgregarTrampa(int x, int y)
    {
        ...
        {
            trampas.Add((x, y));
            laberinto[x, y] = '▲'; // Símbolo de trampa
        }
    }
